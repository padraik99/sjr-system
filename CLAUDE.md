...
|---------|-------|--------|--------|--------|
| Patrick | Volleyball | L4-L5 spinal rehab (foraminal stenosis) | **MAJOR RE-FLARE Jun 19** (cumulative training load) — L5-S1 nerve root, extension-bias/McKenzie Phase 1 active, no strengthening/flexion/core this phase · MBB done Jun 5 → only ~30% relief ×4hr (NEGATIVE facet block) · Bansal (GP) **Jul 8**, Kataria (pain mgmt) **Jul 28**, Matt (PT) after Jul 28 · **primary app user going forward** | King of the Court, Summer 2026 |
| Shaylan | Sprint/Jump (MJC → **UC Berkeley fall 2026**) | **R navicular stress fx (nondisplaced, MRI 6/15/26)** | **Sep 5: ~wk 12 post-MRI. Out of boot early Aug; Berkeley trainer advised another week in the boot for occasional BAREFOOT discomfort.** NWB 6 wks (boot+crutches) Jun–Jul → progressive WB wk 6–8 → PT return-to-run wk 8–14, clearance projected ~late Sep · core+upper cleared, leg tier cleared in writing, swim cleared (pull buoy + ankle strap, no kick) · RED-S neg · dietitian ref (confirm it carried to Berkeley) · **OPEN SINCE JUNE: was bony union ever confirmed by imaging, or was boot removal clinical only? CT, not MRI** · `boot_phase_v1` (v1.3) + `movement-guide` + `Shay_Navicular_Injury_Record_20260905.md` | 11.29 100m PR · State champion 100m · 2nd 200m + LJ |
...
| `shaylan_boot_phase_v1_20260616.html` | **v1.1** | 6-wk strict-NWB navicular block (R navicular SF, nondisplaced, MRI 6/15 · Falcocchia/KP) — core + controlled upper-body lifting cleared; 4-day template (push/NWB arm-bike conditioning/pull/hypertrophy+mobility) + 6-wk volume progression; leg tier (L-leg loaded + R-leg open-chain knee/iso, zero foot/ankle load) **verbally cleared, reconfirm in writing**; pool ruled out; bone-fueling note (RED-S neg, dietitian ref); reexam-prep checklist (imaging-before-WB, return-to-run protocol) · medical-deference framing · **v1.1 (Jun 17): cross-education built in** — "Why the legs don't rest" section (neural contralateral protection + navicular tension-vs-weight explainer: tib post = only tendon on navicular → resisted ankle/foot work loads it even NWB → right foot/ankle stays passive), leg tier upgraded to eccentric-biased heavy LEFT-leg (cross-ed engine) + foot-passive right iso + quad NMES (ask-PT) + NMES reexam Q. Filename unchanged (stable link); footer reads v1.1 · **v1.2 (Jun 23): integrated WRITTEN PT clearance** — leg tier amber→**green (cleared in writing)**; new **Daily Core+Glute Anchor (7×/wk)** card (SL bridge+ISO hold 10–45s/side +1 rep/wk, modified side plank+hip abduction same scheme, dead bugs incl double-leg ext 3×10); leg tier (3×/wk) reworked to PT doses — L-only SL leg press 3×10, SL knee ext 3×10 each (L heavy/R open-chain light), L SL ham curl 3×10, Nordics 3×10 bilateral, **L-only SL heel raise 3×25** (R calf explicitly OUT — plantarflexion pulls navicular), R quad+glute iso, R quad NMES (still ask-PT); new **"Bonus ROI" section** — banded open-chain hip/glute-med series (cleared), **BFR card (ask-PT)** for R-thigh disuse-atrophy mitigation + L low-load (Zhao 2020 BFR+EMS, Hughes review; std in ACL rehab), **swimming reversed from "ruled out" → cleared** but pull-buoy-dominant w/ kick caveat (flutter/dolphin = ankle plantarflexion = tib post pull; kick limits still ask-PT); Day-2 conditioning cue + reexam checklist + disclaimer updated (dropped "pool ruled out"); footer→v1.2. Pure HTML content doc (no JS); tag-balanced, written to disk via bash-rebuild (Edit-tool desync hit the Yari HTML this session — see watch-out). **v1.2.1 (Jun 23): equipment on hand folded in** — Patrick confirmed they own a **small TENS/EMS unit, occlusion cuffs, and bands** (for Shay AND Patrick). BFR/NMES reframed from “ask about” → equipment-ready; asks now PT-settings-only (EMS-mode intensity for a visible contraction — sensory TENS ≠ muscle building; cuff limb-occlusion %); reexam + intro updated; footer→v1.2.1 · **v1.2.2 (Jun 23): swim = ankle-strap no-kick** (Patrick's line — pull buoy + ankle band binding the ankles so kicking is physically impossible, not willpower-based; swim box + reexam item + footer updated). **v1.3 (Jun 23): self-serve demo links + cache-bust refresh** — ported the Patrick-guide demo pattern to the (previously script-less) boot guide: per-exercise **＋ demo** button → prompt → localStorage `sh_demos` (http(s)-validated, ✎ change / empty-removes, 🔍 YouTube-search fallback), injected onto `td.ex` rows by a small post-load script (strips `.cue`/`.pill` for the name key). **Decision (Patrick, Jun 23): self-serve localStorage model, NOT central hardcode** — Shay saves her own links from his unlisted playlist on her device; his picks don't transfer (per-device, accepted; surfaced the gotcha). Added ↺ refresh FAB + no-`?t=` cache-bust redirect (parity w/ other guides). Boot guide now has its first `<script>` but stays **DB-exempt/offline-safe** (localStorage only, no Supabase, no login). node-checked; fixed a quote-escape bug in the FAB onclick post-write. Filename stable; footer→v1.3. Suggested commit: `Shaylan boot v1.3: self-serve per-exercise demo links (localStorage sh_demos) + cache-bust refresh` |
| `Cadence_Weekly_v3_20260402.html` | v3.2 | Supabase · SHIN_COMP · MULTIPLANAR_LOWER · EX_INFO entries |
| `SJR_Yari_Guide_v2_20260414.html` | **v2.8** | W5–W9 added (May 18–June 21) · Hip CARs + Ankle CARs + 90/90 Hip Switch + Lateral Lunge + Copenhagen Plank (Harøy 2019) + Conor Harris glute med (ant/post fibers) + Half-Kneeling Chop across all new weeks · Library nav link added · **v2.8 (Jun 23): W10 "Power Expression" (index 10, Jun 22–28, her final week — departs Fri Jun 26)** — light-load power/velocity gym days Mon (bilateral/ballistic: trap-bar jump ~30% 1RM, KB swing, box jump stick, MB rotational throw) + Wed (unilateral/frontal-plane: lateral bound stick, SL box hop, hip-thrust speed reps, MB OH throw); Thu = **short ball-plyo** (gold: broad-jump→first-touch, SL lateral hop→header, depth-drop→reactive pass, lateral-shuffle→cut-with-ball, ~45 contacts, Verkhoshansky/Elliott); Tue easy tempo+strides, Fri send-off primer, Sat travel, Sun rest. Power MED = low vol/max intent/full recovery (Zourdos); frontal-plane landings framed as ACL-protection-plane reps. Joint-prep backbone kept. Footer→v2.8; +Verkhoshansky/Elliott/Zatsiorsky credits. node-checked (one-script rule held). Suggested commit: `Yari v2.8: add W10 Power Expression — light-load power Mon/Wed + short ball-plyo Thursday (final week before departure)` |
| `SJR_Library_Master_v4_20260402.html` | **v4.4** | 82 cards · quality tags · filter FAQ panel · +Wall Sit Co-Contraction Ladder (Grey) · +Sciatic Nerve Slider / Neural Glide (Mobility & Joint Prep) · top nav (referrer-aware ← Back · Dashboard · Program Overview) · **v4.3 (Aug 17): +Section 12 "Movement & Restoration" (10 cards) — foot revival (toe yoga, short-foot/arch doming, toe splay, barefoot terrain), ankle/toe CARs, movement snacks (deep resting squat, ground sitting/transitions, passive hang, spinal waves). Movement-school cites: Bowman (foot health), Ido Portal, Le Corre/MovNat, Starrett, Spina/FRC. Built for Shaylan's post-boot "movement not work" ask but shared for all; deep-link `#restoration`. Inserted before the references block; 12 sections balanced, filter script intact, footer→v4.3.** Suggested commit: `Library v4.3: +Movement & Restoration section (10 cards) — foot revival, CARs, movement snacks for post-injury restoration` · **v4.4 (Aug 22): deep-link scroll fix (committed)** — guide links like `…#restoration` were landing at page top: on a long page with async web fonts the browser jumps to the hash before layout settles, and `applyFilters()`/`loadFromURL()` stripped the hash by `replaceState`-ing to a bare path. Fix: preserve `location.hash` in the `replaceState` newUrl + re-scroll to the hash target after `fonts.ready`/`window load` (80ms). Fixes every `#section` deep-link, not just restoration. `node --check` clean, one-script rule intact. Suggested commit: `Library v4.4: deep-link #hash scroll fix — preserve hash in replaceState + re-scroll after fonts/layout settle` |
| `SJR_WeeklyGuide_Patrick_v5_20260402.html` | **v7.2** (committed `d8815ed`) | **CORRECTION (Aug 18): the repo file IS v7.0** — the "v6.0.x / phone-only" note from Aug 17 was wrong, caused by a stale mount read; re-connecting the folder showed the true v7.0 (hybrid console, grouped 0–10 log, doctor summary, /10 gates, Nerve-radicular-umbrella all live). Lesson: re-request the directory at session start to force a fresh read; don't trust the first read after a gap. · **v7.1 (Aug 18): medium-day light-routine feature** — in `renderDay`, when `breaches.morningReduce` (Back 3.5–5.5) **OR** `breaches.nerve` (>3.5), and not a Day-C/rest day, a static "Medium Day — Spine-Friendly Swap" card renders above the authored plan: banded glute-med series, sciatic nerve slider, backward jog/treadmill, Movement & Restoration (with a deep-link to Library `#restoration`), plus a "skip the heavy lifts, earn them back on a green day" note. Static card (no RENDER_NAMES/demo-index interaction), authored plan still shows below for reference. Contrast fix (`--muted`/`--dim` lightened) also present. `node --check` clean, one-script rule intact. Suggested commit: `Patrick v7.1: medium-day spine-friendly swap card (back 4–5/10 or nerve up → light band/neural set instead of heavy lifts)` · **v7.2 (Aug 22, committed):** (1) **rehab-done toggle moved Notes → Treatment group** — the "Rehab exercises done today?" No/Yes now sits under the Medrol chip alongside the nexwave/shockwave modality tracker; same element IDs (`seg-exdone-0/1`, `stageExDone`, `exDoneVal`) + same `pt_daily_notes` `ex` storage, so render/save/Doctor-Summary paths are untouched; treatment count badge now includes a staged exercise-done; Notes group is free-text only. (2) **Med-day swap card Search-Demo links** — banded glute-med series + sciatic nerve slider get `🔍 Search Demo` (reusing the existing EX_INFO search URLs so wording stays consistent); backward jog left bare (Patrick's call); CARs covered via the now-fixed Library `#restoration` deep-link (each restoration card carries its own Search Demo). (3) **hdr label fixed** — `hdr-sub` was stuck at v7.0 though the in-file VERSION header already read 7.1 (why Patrick's phone showed 7.0); now v7.2. `node --check` clean, one-script rule + textContent rules intact, no schema change. Suggested commit: `Patrick v7.2: rehab-done toggle → Treatment group; med-day swap Search-Demo links (banded glute-med, sciatic slider); fix header label stuck at v7.0` |
| `shaylan_weekly_v3_20260402.html` | **v3.4** (committed `659bbe9`) | **GPP reset (Aug 18):** guide was populating stale competition-peak content because past the authored 9-wk ramp `detectCurrentWeek` returned null and it showed the last-stored `sh_week`. Added a **`GPP · Return` week** (marked `gpp:true`, last in WEEKS) — post-injury base block decoupled from the meet calendar (general strength, tissue tolerance, gradual walk-jog return, single-leg reintro, Movement & Restoration, foot/shin-aware; bridge until Berkeley's program). `getWeekStart` special-cases the gpp week → **this week's Monday** (so it shows current dates), and `initApp` lands on the gpp index past-horizon. Verified: date sim lands today in the GPP week; `node --check` clean. **Also (Aug 18): persistent 🧘 Movement quick-link** — fixed `.move-fab` anchor → Library `#restoration` (distinct class so `updateInjuryGate` doesn't rewrite its href like it does `.browse-btn`); gives Shay a direct option to the movement work instead of it only living in the Library. Suggested commit: `Shaylan v3.4: GPP · Return block + auto-land past meet calendar + Movement quick-link to Library #restoration` |
| `SJR_Yari_Guide_v2_20260414.html` | **v2.9** (committed `8eb8906`) | **ROLLING RESET BUILT (Aug 18).** Was stale-frozen on the Mexico timeline (11 weeks, BASE Mon Apr 13, last authored "Power Expression" Jun 22–28). Context: **Yari never trialed in Mexico** (agent was winging it) → needs to hold match fitness until a trial. Added **two alternating rolling weeks** to WEEKS + DAYS_BY_WEEK (index 11=A, 12=B, both `roll:true`): **Maintenance · Trial-Ready A (2 runs: extensive tempo + RSA/agility)** and **B (3 runs: aerobic interval + RSA/COD + game-simulation)** — strength 1–2×, daily joint prep/CARs, recovery, ACL-smart deceleration throughout (post-ACL conservative mgmt, R leg dual kick+plant). `mondayOfThisWeek()` + `rollingWeekIndex()` (parity = weeks-since-BASE % 2 → A/B); `getWeekStart` special-cases roll weeks → this week's Monday; `detectCurrentWeek` skips roll weeks in the date loop and returns the parity-correct rolling week past-horizon (auto-lands, current dates). Verified: `node --check` clean, one-script rule, parity sim lands Week A this week → B next. Suggested commit: `Yari v2.9: rolling Maintenance · Trial-Ready block (A 2-run / B 3-run, alternating) + auto-land past the Mexico meet calendar` |
...
| `patrick_protocol_v2_20260402.html` | v2 | Floating This Week button · pre-flare (Mar 2026), now flagged stale via red banner + nav link pointing to the new flare-protocol page — due a full refresh once Jul 8/28 appointments land |
| `patrick-active-flare-protocol-v1-20260702.html` | **v1 NEW** | Static content page (no JS, no Supabase), same dark-theme CSS system as the March protocol doc. Built from the Jul 1–2 pain-mgmt/rehab session: current flare status + care team (Bansal Jul 8, Kataria Jul 28) · Phase 1 daily protocol (morning dead hangs→prone lying→press-ups→sciatic glide→standing ext; afternoon NexWave+repeat; evening reduced press-ups+diaphragmatic breathing) · dead hang decompression rationale · sciatic nerve glide steps + stop criteria · NexWave TENS-only pad/mode/timing spec (manufacturer diagram: low back+leg = TENS, not IFC) · Canal Walk backwards-walking protocol + CNS/novel-motor-pattern rationale + 1-week non-response progression gate · Strict Phase 1 exclusions table (piriformis/hamstring stretch, flexion, knee-to-chest, core, hip strengthening — all Phase 2) · Phase 2 locked placeholder cards · traffic-light response guide (green/yellow/red on leg-pain direction). Linked from March protocol doc nav + red staleness banner, and from Weekly Guide via new ⚠ FAB. **Same-day update (Jul 2, 2nd carryover):** new Toe Tingling section (big toe/L5, position-dependent, progression marker, stand+10-extensions rule, tracked against Medrol days 3–5 for edema/ESI-candidacy signal) · Medrol Dose Pack section (6-day taper 6/5/4/3/2/1, starts Jul 3, before noon, ibuprofen suspended/famotidine continues, NP-friend bridge script) · expanded Bansal 7-item and Kataria 5-item numbered agendas (tamsulosin/neurogenic bladder referral ask, TCA evidence citation — Finnerup 2015/NICE NG59 — vs weak baclofen+gabapentin support, Kaiser fragmented-care note) · refined NexWave zone spec (mirrored Zones 2/3 to left, CH1/CH2 drop to L3, 3-pad Zone 3 calf/ankle/malleolus setup, HMD-mode nerve-accommodation rationale). Suggested commit: `Patrick: add active-flare-protocol-v1 (Jul 2026 flare) + flag March protocol doc stale, link forward` then `Patrick flare-protocol v1.1 + Weekly Guide FAB: Medrol taper, toe tingling, expanded Bansal/Kataria agendas, NexWave zone detail` · **Same-day v1.2 (Bansal responded, out-of-town): stay-the-course callout** — green clinical-box ahead of the Bansal agenda documenting GP's message: continue current regimen unchanged until Jul 8, track wake times/overnight pain/AM dizziness for a data-driven sleep+radicular dose decision. Daily Log gained matching columns (Dizzy/Near-Faint Y-N-S replacing the old Stood-Immediately column, plus Wake Time(s) and Overnight Pain 0-10) — Y/S days conditionally highlighted red/yellow for the visit. Suggested commit: `Patrick flare-protocol v1.2 + log: Bansal stay-the-course note, dizziness/wake-time/overnight-pain tracking` |
| `SJR_WeeklyGuide_Patrick_v5_20260402.html` FAB addition | v6.0 (patch) | Added `.flare-fab` (⚠, red border, positioned left of the library FAB) linking to the Active Flare Protocol page — CSS-only + one anchor tag, no JS touched, one-script rule intact. |
| `SJR_WeeklyGuide_Patrick_v5_20260402.html` Pain Log expansion | v6.0.4 (committed) | **Daily log moved in-app instead of a separate spreadsheet** (Patrick's call — sensitive health data shouldn't sit in the public repo as a downloadable xlsx; reuses the existing private Supabase `pain_logs` sync instead). Added 3 metrics to the existing Pain Log modal, same pattern as Back/Nerve/Glute/Ankle — zero schema changes, same `sbFetch`/`saveLogToSupabase` path: **Overnight** (0–5 pain scale, reused 6-button grid), **Toe Tingle** (Y/N, 2-button `narrow-2` grid), **AM Dizzy** (No/Some/Yes, 3-button `narrow-3` grid, color-escalates to red on "Yes" — this is the one Bansal explicitly asked to track after the syncope episode). `METRICS_NO_TIMING` set hides the Pre/Post-Session toggle for these three (they're not session-relative) and forces `timing:'am'`. Added a **Wake Time(s)** free-text field (localStorage-only, `pt_wake_times`, date-keyed, 35-day auto-prune) since it's once/day free text and doesn't fit the numeric `pain_logs` schema — `saveLog()` now saves this independently so a wake-time-only entry (no metric staged) still persists and closes the modal. Deliberately did NOT add a "Centralization" metric — treated as a clinical interpretation to make from the leg-vs-back trend at the visit, not a raw daily input. `node --check` clean on extracted script, one-`<script>` rule intact, visually re-verified modal markup nesting (flat div-count regex isn't a valid balance check on this file — it mixes static HTML with JS-string-generated HTML). Suggested commit: `Patrick v6.0.4: add Overnight/Toe-Tingle/AM-Dizzy to Pain Log modal + Wake Time(s) field — daily tracking moves in-app, off the public repo` |
| `SJR_WeeklyGuide_Patrick_v5_20260402.html` console + log redesign | **v7.0 (committed)** | **Built Jul 4 per `guide-redesign-build-spec-20260703.md` (approved).** (1) **Hybrid console** — one surface replaces wave headline + gate banner + VBT head + flareup head: eyebrow (wave-light + wave-pos + date), Playfair serif verdict, why-drawer (Ankle row → Glute), action rows (Log day / Log an issue / Performance data / Doctor summary), status chips (lift readiness · flare day · gate ≤3.5/10). VBT body + flareup body kept verbatim as chip-driven expanders — **every JS-updated element ID preserved**, zero changes to governor/VBT/flare render functions except paintWhy (why-glute) + con-date fill. (2) **Grouped log modal, 0–10 half-point sliders** — Pain (Back/Nerve/Glute, **Ankle retired** — merged into Nerve, historical rows kept; single timing toggle re-times all staged pain metrics), Symptoms (leg_dominant 0/1/2 seg absorbing centralization, tingle, dizzy), Sleep (overnight slider, **wakings stepper** — new metric, wake-times text), Treatment (**modality-agnostic** — nexwave/shockwave + ＋custom via prompt→slug, persisted `pt_treat_mods`; before/after sliders keyed per modality as `<mod>_before`/`<mod>_after`, auto-delta; **Medrol chip auto-derived Jul 3–8, self-expires**), Notes (exercise-done toggle + free text → `pt_daily_notes` date-keyed, 35-day prune). Old selectMetric/selectScore machinery deleted; `METRICS_NO_TIMING` → `TIMED_METRICS` (inverted). No schema change. (3) **Thresholds rescaled to true /10** (old×2−0.5): getDayType ≥5.5→C, getLoadModifier ≥5.5→0/≥3.5→0.7, getBreaches morning ≥5.5 / reduce 3.5–5.5 / nerve >3.5, banners + history + gate hints updated, **flare tracker isFlare ≥5.5, recovery ≤3.5, Issue severity (still /5 input) normalized ×2−0.5 inside computeFlareupEpisodes** so dayMax compares like with like. (4) **Doctor Summary modal** — in-app xlsx-Summary replacement for Bansal 7/8 + Kataria 7/28: 7/14/30-day ranges; pain averages (nerve avg includes legacy ankle rows), direction counts + latest, tingle/dizzy day-counts, wakings avg/max, wake times, per-modality before→after deltas, Medrol note, rehab compliance + notes; DOM-constructed (user text via textContent per audit rules); **Print/PDF** via `body.print-summary` + @media print (white bg, chrome hidden). (5) **Demo links extended** — info-less rows (rehab/flare movements) now get a detail drawer with ＋demo + 🔍 YouTube-search fallback (encodeURIComponent — attribute-safe). (6) saveLog now also calls renderFlareupPanel. (7) **v7.0.1 same-day: 'pm' timing added** (Patrick's ask — "burning all day" + delayed TENS response fell in the AM/pre/post hole): 4th toggle button (labels shortened Pre/Post, grid 3→4 cols), TIMING_LABELS/syncTimingButtons/history map updated, Doctor Summary gains conditional Back (PM) + Nerve (PM) rows. Note: governor reads latest log ≤24h regardless of timing — a high PM entry governs next morning until the AM log lands (intended). Header → v7.0. `node --check` clean; onclick↔function + getElementById↔markup cross-checks clean; one-script rule intact. **GO-LIVE: run the scoped conversion SQL (below in queue #4) then open the guide ONLINE once** so converted remote rows overwrite the old-scale localStorage cache (loadLogsFromSupabase is read-only — no duplicate-push risk; verified). Suggested commit: `Patrick v7.0: hybrid console + grouped 0–10 log (Ankle→Nerve), doctor summary, treatment tracker, demo links on rehab rows — thresholds rescaled` |
| `SJR_Periodization_Master_v2_20260313.html` | v2 | Library links fixed to absolute URL v4.1 |

| `Shay_Navicular_Injury_Record_20260905.md` | **NEW (Sep 5)** | **Consolidated injury record for the R navicular stress fracture** — reconstructed from commit history after the originating chat became unretrievable (search surfaced snippets; the thread itself would not open). Pulls together `Shay_PT_Questions_20260615.md` (`949ca56`), `shaylan_boot_phase_v1_20260616.html` (`ee0fb39` → v1.3 `f722cd4`), `Shay_CrossEducation_Evidence_20260617.md` (`e2a0282`) and `shaylan-movement-guide-v1-20260817.html` into one document: diagnosis + high-risk classification (watershed central third), pre-diagnosis competition history (3 meets incl. State finals), the two governing lines (*pain-free ≠ healed*; tib post = only tendon on the navicular → tension loads it at zero body weight), the June treatment timeline, the boot-phase program and its deliberate exclusions, cross-education citations (Farthing 2009 · Carr 2025 · Manca 2021 · Dirks 2014 · Zhao 2020/Hughes), bone-health status (RED-S neg, dietitian ref), Sep 5 status, and the **open union-imaging question from the six-week reexam checklist that the record never shows answered**. Also flags a conflict to resolve: the Aug 17 movement guide prescribes barefoot foot-revival work, which is the same loading mode now producing symptoms. Twin copy in the claude.ai project at `claude/Shay_Navicular_Injury_Record_2026.md` — keep in sync. Suggested commit: `Shay: add consolidated navicular injury record (reconstructed from ee0fb39 chain) — diagnosis, timeline, program, open union-imaging question` |
...
### Next-session queue (as of Jun 22 2026)
1. **Supabase security** — STEP 0 lockdown SQL run (idempotent version); decision = shared single login. Plan: `supabase-auth-migration-plan-20260622.md`. Next: build the login gate (pilot Patrick guide → roll out → flip policies to `authenticated` last). **Sticky session is a must-have** — Shay (Berkeley) + Yari (Mexico) are staying on the guide remotely, so: one password, once per device, then silent refresh.
2. ~~**Shaylan** — feed in written PT clearance; expand boot guide beyond NWB-only template.~~ ✅ **DONE Jun 23 (boot v1.2, committed).** Leg tier now written-cleared (amber→green); added daily core/glute anchor + expanded leg tier to PT doses + Bonus ROI section (band/BFR/swimming). **Equipment on hand (Patrick, Jun 23): small TENS/EMS unit + occlusion cuffs + bands — for Shay AND Patrick.** So BFR/NMES are equipment-ready; **only PT settings remain ask-PT** (EMS muscle-mode intensity for a real contraction vs sensory-only TENS; cuff pressure %), plus swim-kick limits — all in the reexam checklist. **Possible discrepancy to confirm with Patrick:** swimming was previously "MD ruled out" in v1.1 — v1.2 reverses it per the new written clearance; programmed conservatively (pull-buoy, no R-foot kick) pending PT confirmation of kick mechanics.
3. ~~**Yari** — extend the guide one more week (W10): light-load power + Thursday ball-plyo.~~ ✅ **DONE Jun 23 (v2.8, committed).** W10 "Power Expression" Jun 22–28 built: power Mon/Wed, ball-plyo Thu, send-off Fri. Suggested commit in inventory row above.

### Next-session queue (updated Jul 4 2026)
4. ~~**Patrick guide console + log redesign**~~ ✅ **BUILT Jul 4 (v7.0, committed).** Full detail in the inventory row above. **Patrick's go-live steps, in order:** (a) push v7.0 via GitHub Desktop; (b) run the scoped conversion SQL in the Supabase SQL editor: `UPDATE pain_logs SET score = GREATEST(0, score*2 - 0.5) WHERE metric IN ('morning','nerve','glute','ankle','overnight');` — pain metrics ONLY, never tingle/dizzy; (c) open the guide online once so the converted rows overwrite the local cache; (d) browser smoke test — console clean, verdict renders, log a test entry per group, open Doctor Summary, print preview. Original spec kept for reference: `guide-redesign-build-spec-20260703.md`.

*(original item for context:)* **Patrick guide console + log redesign — APPROVED, spec locked:** `guide-redesign-build-spec-20260703.md`. Hybrid console (Concept A card+verdict framing + Concept B list-row logging + mono section headers) replacing the 5 stacked surfaces. Grouped **0–10 decimal** log (Pain: Back/Nerve/Glute — **Ankle retired, merged into Nerve** per Patrick, calf/ankle/nerve synonymous now; historical `ankle` rows kept; Symptoms: leg-vs-back direction [absorbs centralization] + Toe-Tingle Y/N + AM-Dizzy No/Some/Yes stay toggles; Sleep: overnight + night-wakings# + wake-times; Treatment: **modality-agnostic** before/after tracker for NexWave AND shockwave (if re-borrowed), Medrol-day auto-derived/self-expiring 7/8; Notes). In-app **export/summary** view (recreates the `patrick_pain_rehab_log.xlsx` Summary tab for Bansal 7/8 + Kataria 7/28 — moves health data off the loose xlsx onto private sync). Demo links kept + **extended to rehab/flare rows**. ✅ **Contrast fix applied** (`--muted` #8a8680→#b0aca2, `--dim` #5a5855→#8a8680). Go-live SQL (scoped): `UPDATE pain_logs SET score=GREATEST(0,score*2-0.5) WHERE metric IN ('morning','nerve','glute','ankle','overnight')` — NOT tingle/dizzy. Build order + watch-outs in the spec. **NOTE:** the standalone `patrick_pain_rehab_log.xlsx` (uploaded this session) is the source for the rehab fields being folded in.
...
- Med schedule: gabapentin ~9pm **before** cyclobenzaprine 11pm finisher (pharmacist pref, ~2h stagger). **Magnesium glycinate rides with the 11pm finisher, NOT with gabapentin** — Mg binds gabapentin in the gut and cuts its absorption, so keep them ≥2h apart (gabapentin first). Daytime relaxation only if bedtime carryover (~18h t½) insufficient → prefer low-dose **baclofen 5mg** (less sedating, not anticholinergic, doubles as baclofen taper) over daytime ½-tab cyclobenzaprine.
...
**Shipped to repo this session (committed):**
...
- `SJR_WeeklyGuide_Patrick_v5_20260402.html` → **⚠ flare-fab repointed** from the full Active Flare Protocol to the new daily cheat card (`patrick-flare-cheatcard-20260709.html`, absolute URL; cheat card links onward to the full protocol). CSS-only href/title change, no JS touched, one-script rule intact (verified: 1 `<script>`). Commit: `Patrick WG: repoint ⚠ flare-fab to daily flare cheat card (full protocol still linked from the card)`
...
Branch in sync with origin/main through `8bab5f7` (June 15 2026). The v5.12 / Library-v4.2 work below has since been committed (working tree verified clean at `97c383f`, Sep 4 2026). (Git index had corrupted mid-session via the sandbox; rebuilt with `git reset` June 15.)

| Hash | Message |
|------|---------|
| _pending_ | **Patrick v7.0** (committed): hybrid console + grouped 0–10 log redesign. Suggested commit: `Patrick v7.0: hybrid console + grouped 0–10 log (Ankle→Nerve), doctor summary, treatment tracker, demo links on rehab rows — thresholds rescaled` · Note (Jul 4): everything below through v6.0.4 + contrast fix has LANDED on origin/main (`947eb36`, `75dac44` etc.) — Patrick pushed between sessions; rows kept for history |
| _pending_ | **Patrick v6.0.3** (committed): Gate-banner trim. Wave headline `＋ Log` and the gate cluster `Log` button both called `openModal()` — duplicate pain-log entry. Removed the gate `Log`; wave `＋Log` (line ~528) is now the sole pain-log entry point. Collapsed the gate cluster from two rows to one: ⚠ Issue · 🎯 Ready · 📊 Data. Suggested commit: `Patrick v6.0.3: drop duplicate gate Log button, collapse gate cluster to one row (Issue/Ready/Data)` |
| _pending_ | **Patrick v6.0.2** (committed): `.modal` bottom-sheet was unbounded (parent `align-items:flex-end`, no max-height/overflow) → tall content (Pain/Issue/Data modals, shared class) overflowed off the TOP of the viewport, clipped under the backdrop blur and unreachable on mobile until a resize. Added `max-height:90vh; max-height:90dvh; overflow-y:auto; -webkit-overflow-scrolling:touch;`. `dvh` accounts for mobile browser chrome; `vh` fallback. Suggested commit: `Patrick v6.0.2: cap .modal height to viewport + scroll overflow (fixes off-top clipping of tall modals on mobile)` |
| _pending_ | **Patrick v6.0.1** (committed): Flareup tracker trigger widened from Back-only to ALL pain-log metrics (Back/Nerve/Glute/Ankle ≥3) + Issue regions back/hip/knee/shin/foot ≥3. Was showing empty while Patrick logged Nerve/Glute/Ankle ≥3 since Jun 12 — only `metric==='morning'` and back/hip injuries counted. `computeFlareupEpisodes` now keys on a per-date MAX symptom score (recovery requires *all* inputs ≤2 on two consecutive logged days — a high nerve day can't be falsely resolved by a low Back log). Glute included deliberately (Patrick: high glute reading = pain-driven guarding/AMI, not just weakness). Suggested commit: `Patrick v6.0.1: flareup tracker counts all pain metrics (Back/Nerve/Glute/Ankle) + lower-quarter Issues, not Back-only` |
| _pending_ | **Patrick v6.0** (committed): Wave-periodization engine + downregulate-only governor + headline-verdict UI + dating-freeze fix + init() TDZ bugfix. Suggested commit: `Patrick v6.0: date-driven wave engine + green-day governor; kill dating freeze; move init() past vbtLog TDZ` |
...
### 🔴 RLS / auth — Supabase alert Jun 22 2026 → plan: `supabase-auth-migration-plan-20260622.md`
- Supabase flagged: `public.subjects` RLS OFF (CRITICAL — turned out to be a **stray StudyLab table** mistakenly run into this project; lock via `enable RLS`, then drop once StudyLab's own copy confirmed) + `pain_logs` always-true write policy.
- Root truth: no-auth app (anon key in PUBLIC repo + page source) → reads open to anyone, "always-true" write policies unavoidable without auth.
- **Decision (Patrick, Jun 22): build SHARED SINGLE LOGIN** (one credential gates the DB; flip policies `anon`→`authenticated`). NOT per-user isolation (coach sees all; roster dispersing to ~just Patrick). No `athlete_id` rewrite.
- **STEP 0 lockdown SQL ready to run today** (independent): enable RLS on `subjects`; replace blanket policies with op-scoped ones — pain/injury = insert+select only (kills destructive update/delete surface), athlete_state = insert+update+select (upsert), assessment_logs = +delete. Code-verified against actual table usage.
- Migration: dashboard (enable email auth, create 1 user, DISABLE signups) → code (raw-REST login gate + `sbFetch` adds `Authorization: Bearer <user JWT>` alongside `apikey`; refresh on 401) → pilot on Patrick guide → roll to other DB files → **flip policies to `authenticated` LAST**. Offline Shaylan boot guide exempt (no DB). Full steps/gotchas in the plan doc.
- Status: plan written; STEP 0 + build not yet executed.
...
- **★ MAJOR FLARE + SYNCOPE — Jun 19–20 2026** (full write-up: `patrick-flare-syncope-20260620.md`, repo root). Overload cascade: Thu Jun 18 solo 4×4 took ~2 intervals to reach pace (low readiness from the off) → glute sore + back tight → Fri Jun 19 "easy" field session but **high volume of firm ball strikes** (volume load, not max) → back tightened → sauna 2×10 min + **skipped lunch/small supper** (dehydration + underfueling) → bedtime **Tylenol PM (acetaminophen+diphenhydramine) + regular Tylenol** → ~4:30 am woke in severe pain, couldn't roll either way, stood → sweating + vision tunneling → **syncope (LOC), found by wife on floor**; minor head contact (fall broken by laundry basket/dresser). Sat Jun 20: ambulatory (2 slow miles, stairs OK), prone fine, sitting + sit-to-stand brutal. **Escalation vs ~2 wks ago: more sore + nerve pain now in the CALF (peripheralizing distally → more nerve-root irritation, reinforces foraminal/radicular generator).** Red-flag screen NEGATIVE (no saddle anesthesia, no bowel/bladder change, no bilateral weakness). Syncope read = multifactorial vasovagal/orthostatic (pain + sauna dehydration + underfueling + diphenhydramine + rapid AM standing) — likely benign but **first witnessed LOC w/ fall → advised to call care team this week, not wait for Jul 28**. Two items for the team: (1) the syncope + acetaminophen load; (2) distal radicular escalation may inform ESI-vs-RFA timing. **Coaching: hard stop — no 4×4/gym/testing; gentle walking + prone + refuel/hydrate.** This is the green-day governor's whole thesis in one bad weekend: consecutive loading through low-readiness signals (lagged glute/back warnings) → spike → flare. **Sun Jun 21 update:** no 2nd syncope but pain as brutal; back "locked up," glute wired for max pain with rotation + certain hip-flexion ranges, tiny-step antalgic gait. **Restarted prescription baclofen + gabapentin** (off them previously) — flag for future training: gabapentin can blunt proprioception/balance (existing research bookmark below), so single-leg/balance work stays paused while on it; baclofen + gabapentin (+ any Tylenol PM diphenhydramine) = stacked sedation/hypotension the day after a faint → fall risk. **Syncope did NOT recur:** next morning, same wake-in-pain/stand/walk scenario but regular Tylenol only (no diphenhydramine) + better hydrated/fueled → no presyncope. Reproducing the trigger minus the aggravators = no event → supports situational vasovagal one-off (Patrick's read, concur). Faint now a footnote; calf radicular pain is the active signal to raise with the team.
...
### Wave-periodization engine (Patrick guide) — ✅ SHIPPED v6.0 (Jun 16 2026, committed)
