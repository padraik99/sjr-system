...
| _repo-state note (Aug 17)_ | — | **Patrick guide `SJR_WeeklyGuide_Patrick_v5_20260402.html` in the repo is still v6.0.x internally** (old wave-headline, not the hybrid console). The **v7.0 hybrid-console + grouped-log build lives ONLY on Patrick's phone, uncommitted** — never pushed. So the redesign build spec (`guide-redesign-build-spec-20260703.md`) and the medium-day light-routine feature both need Patrick to PUSH v7.0 first, or work will be on the v6 base and risk clobbering the phone version. **The medium-day feature request (Aug 17): on a log of 4–5/10, surface light-load/band/sciatica-friendly work (banded glute-med, sciatic slider, backwards-jog options — all in Library) instead of heavy lifts like hip thrusts.** |
