...
| `patrick-active-flare-protocol-v1-20260702.html` | **v1 NEW** | Static content page (no JS, no Supabase), same dark-theme CSS system as the March protocol doc. Built from the Jul 1–2 pain-mgmt/rehab session: current flare status + care team (Bansal Jul 8, Kataria Jul 28) · Phase 1 daily protocol (morning dead hangs→prone lying→press-ups→standing ext; afternoon NexWave+repeat; evening reduced press-ups+diaphragmatic breathing; once-daily sciatic glide) · dead hang decompression rationale · sciatic nerve glide steps + stop criteria · NexWave TENS-only pad/mode/timing spec (manufacturer diagram: low back+leg = TENS, not IFC) · Canal Walk backwards-walking protocol + CNS/novel-motor-pattern rationale + 1-week non-response progression gate · Strict Phase 1 exclusions table (piriformis/hamstring stretch, flexion, knee-to-chest, core, hip strengthening — all Phase 2) · Phase 2 locked placeholder cards · traffic-light response guide (green/yellow/red on leg-pain direction). Linked from March protocol doc nav + red staleness banner. Suggested commit: `Patrick: add active-flare-protocol-v1 (Jul 2026 flare) + flag March protocol doc stale, link forward` |
