...
| `SJR_Yari_Guide_v2_20260414.html` | v2.8 (examined Aug 18) | **NEEDS A ROLLING RESET (queued, not urgent — Patrick: "can wait a bit").** Same stale-freeze as Shay's old bug: 11 weeks built entirely around the Mexico timeline (BASE Mon Apr 13; last week "Phase 3 · Power Expression" Jun 22–28, "bonus week before Friday departure"). Past that horizon `detectCurrentWeek` returns null → `initApp` falls to `loadWeekState` (stored week) → **frozen on a stale week, NOT rolling.** Context: **Yari never trialed in Mexico** (the "agent" there was winging it); she needs to **hold match fitness until a trial materializes**, so she should be rolling like Patrick. FIX (same pattern as Shay's GPP): add a rolling **"Maintenance · Trial-Ready"** block (ACL-return-safe — she's post-ACL conservative mgmt, R leg dual kick+plant exposure; aerobic base + strength maintenance + RSA/COD upkeep + daily joint prep/CARs, no peak/taper), mark it `gpp:true`/rolling, special-case `getWeekStart` → this week's Monday, auto-land past-horizon. |
